...
from microsplit_reproducibility.datasets import (
...
def find_recent_metrics():
    last = (
        Path("lightning_logs") / Path(sorted(os.listdir("lightning_logs")[:10])[-1])
    ).absolute() / "metrics.csv"
    assert (
        last.exists()
    ), f"File {last} does not exist. Please remove the lightning_logs folder and re-run the training."
    return last
...
def plot_metrics(df):
    ...
    if "reconstruction_loss_epoch" in df.columns:
        df["reconstruction_loss_epoch"].to_frame("recons_loss").dropna().reset_index(
            drop=True
        ).plot(ax=ax[0], marker="o")
    else:
...
def get_highsnr_data(
    train_data_config, val_data_config, test_data_config, evaluate_on_validation
):
