#!/bin/bash
# Train per-channel noise models for cpg0029-chroma-pilot (Step 2).
#
# Submits 5 parallel SLURM jobs (one per channel) or runs sequentially.
#
# Usage:
#   ./noise_models.sh
#   ./noise_models.sh --slurm
#
# Options:
...
echo "================================================================================" >&2
echo "cpg0029 noise models | dataset_dir=${DATASET_DIR}" >&2
echo "================================================================================" >&2
...
#!/bin/bash
#SBATCH --job-name=nm_cpg0029_${CH}
...
echo "Job \${SLURM_JOB_ID}: NM cpg0029 ${CH} | Node: \${SLURMD_NODENAME} | Start: \$(date)"
...
EOF
    ...
    SUBMITTED+=("${JOB_ID}")
    echo "  Submitted NM job ${JOB_ID}: cpg0029/${CH}" >&2
done
