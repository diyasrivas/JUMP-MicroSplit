...
def get_4channel_combinations():
    """Get all 4-channel combinations from 5 available channels"""
    ALL_CHANNELS = ["DNA", "RNA", "ER", "AGP", "Mito"]
    combinations = []
    
    # Loop through all channels and create combinations by leaving one out
    for i, channel_to_exclude in enumerate(ALL_CHANNELS):
        combination = [ch for ch in ALL_CHANNELS if ch != channel_to_exclude]
        combinations.append(combination)
        
    return combinations


def get_channel_dir_from_index(index):
    """Get the channel directory name from the combination index"""
    combinations = get_4channel_combinations()
    if index < 0 or index >= len(combinations):
        raise ValueError(f"Invalid combination index: {index}. Must be between 0 and {len(combinations)-1}")
    
    channels = combinations[index]
    return "_".join([ch.lower() for ch in channels])
...
def main():
    parser = argparse.ArgumentParser(description='Train MicroSplit model for 4-channel combinations')
    parser.add_argument('--index', type=int, required=True, help='Index of the 4-channel combination (0-4)')
    ...
    # Get the 4-channel combinations
    combinations = get_4channel_combinations()
    if args.index < 0 or args.index >= len(combinations):
        raise ValueError(f"Invalid combination index: {args.index}. Must be between 0 and {len(combinations)-1}")
    
    target_channel_list = combinations[args.index]
    
    # Train the model for this combination
    train_model(
