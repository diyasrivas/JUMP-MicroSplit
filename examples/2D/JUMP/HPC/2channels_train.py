...
def find_existing_2channel_dirs():
    """Find existing 2-channel directories in experiments/2_channels"""
    base_dir = "experiments/2_channels"
...
def main():
    parser = argparse.ArgumentParser(description='Train MicroSplit model for 2-channel combinations')
    parser.add_argument('--index', type=int, required=True, help='Index of the 2-channel combination (0-9)')
    ...
    # Find existing 2-channel directories
    existing_dirs = find_existing_2channel_dirs()
    if not existing_dirs:
        print("No 2-channel directories found. Please create the datasets first.")
        sys.exit(1)
