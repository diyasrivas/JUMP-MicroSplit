#!/bin/bash
# MicroSplit training for cpg0000-jump-pilot (Step 3).
#
# Usage:
#   ./train.sh --cell_lines "A549 U2OS"
#   ./train.sh --cell_lines "A549" --test   # 2-epoch sanity check
#   ./train.sh --dependency "12345:12346"   # after noise model jobs
#
# Options:
#   --cell_lines "A549 U2OS"     Space-separated (default: A549 U2OS)
#   --dataset_base DIR           (default: /project/cell_paint_mono/training_datasets)
...
echo "================================================================================" >&2
echo "${MODE_NAME}: cell_lines=${CELL_LINES[*]} epochs=${NUM_EPOCHS} batch=${BATCH_SIZE}" >&2
echo "================================================================================" >&2
...
#!/bin/bash
#SBATCH --job-name=${MODE_NAME}_train_${CL}
#SBATCH --output=${LOG_DIR}/${MODE_NAME,,}_${CL}_%j.log
#SBATCH --error=${LOG_DIR}/${MODE_NAME,,}_${CL}_%j.log
...
echo "Job \${SLURM_JOB_ID}: ${MODE_NAME} train ${CL} | Node: \${SLURMD_NODENAME} | Start: \$(date)"
...
EOF
    ...
    SUBMITTED+=("${JOB_ID}")
    echo "  Submitted job ${JOB_ID}: ${MODE_NAME} train ${CL}" >&2
done
