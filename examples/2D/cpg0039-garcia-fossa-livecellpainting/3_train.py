#!/usr/bin/env python3
"""
MicroSplit training for cpg0039-garcia-fossa-livecellpainting dataset.

Live Cell Painting with Acridine Orange — only 2 channels:
  AO_Green  (GFP filter, 469/525 nm)
  AO_Red    (PI filter,  531/647 nm)

Train one model per cell line (Huh_7, MCF_7, PNT1A, PC_3).
Point --dataset_dir at the directory built by 1_datasets.py for the target
cell line.

Expected dataset layout under <dataset_dir>/:
    AO_Green/   — individual channel .tiff files (uint16)
    AO_Red/
    combined/   — pixel-wise sum of both channels (float32)
...
Usage:
    ...
    python 3_train.py --dataset_dir ... --test_mode
"""
...
# 2-channel Live Cell Painting (Acridine Orange only)
CHANNELS = ["AO_Green", "AO_Red"]


def parse_args():
    parser = argparse.ArgumentParser(
        description="MicroSplit training for cpg0039-garcia-fossa-livecellpainting"
    )
