#!/bin/bash
# MicroSplit training for cpg0039-garcia-fossa-livecellpainting (Step 3).
#
# 2-channel Live Cell Painting (AO_Green, AO_Red). Run once per cell line.
#
# Usage:
#   ./train.sh --dataset_dir .../training_dataset_cpg0039_Huh_7
#   ./train.sh --dataset_dir .../training_dataset_cpg0039_MCF_7 --test
#   ./train.sh --dataset_dir ... --dependency "NM_JOB1:NM_JOB2"
#
# Options:
...
echo "================================================================================" >&2
CELL_LABEL=$(basename "${DATASET_DIR}" | sed 's/training_dataset_cpg0039_//')
echo "${MODE_NAME}: cpg0039 ${CELL_LABEL} | epochs=${NUM_EPOCHS} batch=${BATCH_SIZE}" >&2
...
#!/bin/bash
#SBATCH --job-name=${MODE_NAME}_cpg0039_${CELL_LABEL}
#SBATCH --output=${LOG_DIR}/${MODE_NAME,,}_cpg0039_${CELL_LABEL}_%j.log
#SBATCH --error=${LOG_DIR}/${MODE_NAME,,}_cpg0039_${CELL_LABEL}_%j.log
...
echo "Job \${SLURM_JOB_ID}: ${MODE_NAME} train cpg0039 | Node: \${SLURMD_NODENAME} | Start: \$(date)"
...
echo "Submitted job ${JOB_ID}: ${MODE_NAME} train cpg0039" >&2
echo "${JOB_ID}"
