#!/usr/bin/env python3
"""
MicroSplit plate-level prediction for cpg0036-EU-OS-bioactives Cell Painting data.

For each FOV in a plate:
  1. Reads 4 fluorescence channels from raw .tiff plate images
...
Channel mapping (cpg0036, 4-channel Opera Phenix, EU-OPENSCREEN Cell Painting):
  Mito=ch1, Actin_AGP=ch2, ER_RNA=ch3, DNA=ch4

  The ER_RNA channel contains spectrally mixed ConA Alexa 488 + SYTO 14 signal.
  The Actin_AGP channel contains spectrally mixed WGA Alexa 555 + Phalloidin Alexa 568.
  MicroSplit unmixes these 4 channels from their combined signal.

Use the model trained on the same cell line as the plate being predicted.
...
Usage:
    python 4_predict.py \\
        --plate_dir  /project/cell_paint_mono/cpg0036-EU-OS-bioactives/{batch}/images/{plate}/ \\
        --training_dir /project/cell_paint_mono/training_datasets/training_dataset_cpg0036_HepG2 \\
...
# 4-channel EU-OPENSCREEN Cell Painting
CHANNELS = ["DNA", "ER_RNA", "Actin_AGP", "Mito"]

# cpg0036 channel mapping (Opera Phenix, longest→shortest wavelength ordering)
CHANNEL_MAPPING = {
    "DNA":       4,   # Hoechst 33342          (435-480 nm)
    "ER_RNA":    3,   # ConA Alexa 488 + SYTO14 (500-550 nm, mixed)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MicroSplit plate prediction for cpg0036-EU-OS-bioactives"
    )
